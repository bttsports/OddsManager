...
/// Payload sent by Combined No bot to alert webhook and emitted to frontend.
#[derive(Debug, Clone, Serialize, Deserialize)]
struct CombinedNoAlertPayload {
    pub reason: String,
    #[serde(flatten)]
...
struct CombinedNoConfig {
    ...
    check_interval_sec: u32,
    #[serde(default = "default_max_combined")]
    max_combined: u32,
...
fn default_max_combined() -> u32 { 99 }
fn default_shares() -> u32 { 10 }
...
/// Write Combined No script to path and optionally write installer. Called from frontend
/// after user picks path via dialog.save() (avoids blocking_save_file deadlock on Windows).
#[tauri::command]
fn write_combined_no_script(
    app: tauri::AppHandle,
    path: String,
    script: String,
    config: CombinedNoConfig,
...
const ALERTS_SERVER_PORT: u16 = 8767;

/// Base URL for the Combined No alerts webhook (user can expose via ngrok for VPS bots).
#[tauri::command]
fn combined_no_alerts_webhook_url() -> String {
    format!("http://127.0.0.1:{}/api/combined-no-alert", ALERTS_SERVER_PORT)
}

/// One generated Combined No script from install_combined_no_*.sh.
#[derive(Debug, Serialize)]
struct CombinedNoScriptInfo {
    event_ticker: String,
    script_name: String,
    install_name: String,
    service_name: String,
...
/// List Combined No scripts generated by save_combined_no_strategy_script (scan install_combined_no_*.sh).
#[tauri::command]
fn list_combined_no_scripts() -> Result<Vec<CombinedNoScriptInfo>, String> {
    ...
    for entry in fs::read_dir(&dir).map_err(|e| e.to_string())? {
        ...
        if name.starts_with("install_combined_no_") && name.ends_with(".sh") {
            let stem = name.strip_prefix("install_combined_no_").and_then(|s| s.strip_suffix(".sh")).unwrap_or("");
            let event_ticker = stem.replace('_', "-");
            let script_name = format!("combined_no_{}.py", stem);
            let service_suffix = mm_service_suffix(&event_ticker);
            let service_name = format!("oddsmanager-combinedno-{}", service_suffix);
            out.push(CombinedNoScriptInfo {
                event_ticker,
                script_name,
                install_name: name,
                service_name,
    ...
    out.sort_by(|a, b| a.event_ticker.cmp(&b.event_ticker));
    Ok(out)
}

/// VPS status params for SSH check.
#[derive(Debug, Deserialize)]
struct CombinedNoVpsParams {
    host: String,
    #[serde(default = "default_vps_user")]
    user: String,
}

fn default_vps_user() -> String {
    "root".to_string()
}

/// One service status from VPS.
#[derive(Debug, Serialize)]
struct CombinedNoServiceStatus {
    service_name: String,
    status: String,
    active: bool,
}

/// Check VPS for running oddsmanager-combinedno-* services via SSH.
#[tauri::command]
fn get_combined_no_vps_status(params: CombinedNoVpsParams) -> Result<Vec<CombinedNoServiceStatus>, String> {
    let host = params.host.trim();
    if host.is_empty() {
        return Err("Host is required".to_string());
    }
    let target = format!("{}@{}", params.user, host);
    let output = Command::new("ssh")
        .args([
            "-o", "ConnectTimeout=5",
            "-o", "StrictHostKeyChecking=accept-new",
            &target,
            "systemctl list-units 'oddsmanager-combinedno-*' --no-pager --plain --no-legend 2>/dev/null || true",
        ])
        .output()
    ...
    for line in stdout.lines() {
        ...
        if parts.len() >= 3 {
            let service_name = parts[0].trim_end_matches(".service").to_string();
            let status = parts[1].to_string();
            let active = status == "active";
            out.push(CombinedNoServiceStatus {
                service_name,
                status,
                active,
            });
...
fn run_alerts_server(app: tauri::AppHandle) {
    thread::spawn(move || {
        let server = match Server::http(("127.0.0.1", ALERTS_SERVER_PORT)) {
            Ok(s) => s,
            Err(e) => {
                eprintln!("Combined No alerts server failed to bind :{}: {}", ALERTS_SERVER_PORT, e);
                return;
            }
        };
        loop {
            match server.recv_timeout(Duration::from_secs(1)) {
                Ok(Some(mut req)) => {
                    ...
                    let ok = if method == Method::Post && url == "/api/combined-no-alert" {
                        match serde_json::from_slice::<CombinedNoAlertPayload>(&body) {
                            Ok(payload) => {
                                let _ = app.emit("combined-no-alert", &payload);
                                true
                            }
                            Err(_) => {
                                if let Ok(obj) = serde_json::from_slice::<serde_json::Value>(&body) {
                                    ...
                                    let payload = CombinedNoAlertPayload { reason: reason.to_string(), details };
                                    let _ = app.emit("combined-no-alert", &payload);
                                    true
                                } else {
                                    false
                    ...
                    let resp = Response::from_string(if ok { "OK" } else { "Bad request" }).with_status_code(status);
...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            run_alerts_server(app.handle().clone());
            Ok(())
        })
        .invoke_handler(tauri::generate_handler![
            ...
            write_combined_no_script,
            combined_no_alerts_webhook_url,
            list_combined_no_scripts,
            get_combined_no_vps_status,
