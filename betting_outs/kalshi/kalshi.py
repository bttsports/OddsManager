...
# 1. Query NYC weather market
async def query_nyc_weather_market():
    print("Querying NYC weather market...")
    data = await self.make_request("GET", "/markets", params={"search": "NYC WEATHER", "status": "active"})
    markets = data.get("markets", [])
    if not markets:
        raise ValueError("No NYC weather market found.")
    market = markets[0]
    print(f"Market found: {market['ticker']} | {market['title']}")
    return market["ticker"]



def kalshi_advanced_api_code():
    private_key = load_private_key(PRIVATE_KEY_PATH)
    ws_client = KalshiWebSocketClient(KALSHI_API_KEY, private_key)

if __name__ == "__main__":
    kalshi_advanced_api_code()